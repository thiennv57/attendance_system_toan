...
def expand_schedule_value(schedule_value):
    if not schedule_value:
        return []
    if schedule_value == DOUBLE_SLOT_VALUE:
        return [TIME_SLOT_1, TIME_SLOT_2]
    return [schedule_value]


def build_student_slots_map(student_ids, day_of_week):
    if not student_ids or not day_of_week:
        return {}
    ...
    student_slots_map = {}
    slot_order = [TIME_SLOT_1, TIME_SLOT_2]

    for schedule in schedules:
        student_slots_map.setdefault(schedule.student_id, []).append(schedule.time_slot)

    for student_id, slots in student_slots_map.items():
        student_slots_map[student_id] = sorted(
            slots,
...
def add_student():
    ...
    if request.method == 'POST':
        ...
        for day_num in days_of_week.keys():
            schedule_value = request.form.get(f'day_{day_num}')
            for time_slot in expand_schedule_value(schedule_value):
        ...
        if giaovien1:
            existing_assignment = TeacherStudentAssignment.query.filter_by(
...
def add_quick_student():
    ...
    if not name:
        flash('Tên học sinh không được để trống.', 'danger')
        return redirect(url_for('student_management')) # Redirect back to management page on error
    ...
    return redirect(url_for('student_management')) # Redirect back to management page
...
def take_attendance():
    ...
    if selected_date and selected_slot:
        ...
        if all_student_ids_for_display:
            ...
            student_slots_for_day = build_student_slots_map(all_student_ids_for_display, day_of_week_for_schedule)
    ...
    # Build recorder_present_counts and recorder_colors for template
    recorder_present_counts = {}
    for info in existing_attendance_map.values():
        if info.get('status') == 'present' and info.get('recorded_by'):
            rec = info['recorded_by']
            recorder_present_counts[rec] = recorder_present_counts.get(rec, 0) + 1

    _color_palette = ['primary', 'success', 'danger', 'warning', 'info', 'dark', 'secondary']
    all_recorders = sorted({info['recorded_by'] for info in existing_attendance_map.values() if info.get('recorded_by')})
    recorder_colors = {rec: _color_palette[i % len(_color_palette)] for i, rec in enumerate(all_recorders)}
    current_url = build_current_url()
    ...
    return render_template('take_attendance.html',
                         ...
                         student_slots_for_day=student_slots_for_day,
                         recorder_colors=recorder_colors,
                         recorder_present_counts=recorder_present_counts,
                         current_url=current_url)
...
def add_unscheduled_student_to_attendance():
    ...
    if student_id_to_add:
        ...
        return redirect(url_for('take_attendance', date=date_str, slot=time_slot, search_query=search_query))
    ...
    return redirect(url_for('take_attendance', date=date_str, slot=time_slot))
...
def comment_management():
    ...
    if not ATTENDANCE_COMMENT_SUPPORTED:
        ...
        for student in students:
            students_with_latest_comment.append({
                ...
                'latest_comment': {
                    ...
                    'updated_at': student.comment_last_updated_at
                }
            })

        return render_template(
            'comment_management.html',
    ...
    # Thêm thông tin nhận xét gần nhất từ bảng Attendance cho mỗi học sinh
    students_with_latest_comment = []
    for student in students:
        # Tìm nhận xét gần nhất có nội dung (dựa trên ngày ca học date và time_slot)
        latest_attendance_with_content = db.session.query(Attendance).filter(
            Attendance.student_id == student.id,
            Attendance.comment.isnot(None),
            Attendance.comment != ''
        ).order_by(Attendance.date.desc(), Attendance.time_slot.desc()).first()

        if latest_attendance_with_content:
            latest_comment = {
                'content': latest_attendance_with_content.comment,
                'updated_at': latest_attendance_with_content.date  # Sử dụng ngày của ca học
            }
        else:
            # Fallback nếu học sinh chưa có nhận xét nào có nội dung
            latest_comment = {
                'content': None,
                'updated_at': None
            }
    ...
    return render_template('comment_management.html', students_with_latest_comment=students_with_latest_comment, students_pagination=students_pagination, search_query=search_query, grade_filter=grade_filter, available_grades=all_grades, sort_by=sort_by, sort_order=sort_order)
...
def update_attendance_comment():
    ...
    if not attendance_id or not student_id:
        flash('Thiáº¿u thÃ´ng tin báº¯t buá»™c.', 'danger')
        return redirect(url_for('comment_management'))

    if not ATTENDANCE_COMMENT_SUPPORTED:
        flash('MÃ´i trÆ°á»ng hiá»‡n táº¡i chÆ°a há»— trá»£ nháº­n xÃ©t theo buá»•i há»c.', 'warning')
...
def student_comments(student_id):
    ...
    if ATTENDANCE_COMMENT_SUPPORTED:
    ...
    return render_template(
        ...
        attendance_comment_supported=ATTENDANCE_COMMENT_SUPPORTED
    )
...
def create_test_exam():
    ...
    if request.method == 'POST':
        ...
        if not title:
            flash('Tên bài kiểm tra không được để trống.', 'danger')
        ...
        if not students_in_grade:
            flash('Khối này hiện chưa có học sinh để tạo bài kiểm tra.', 'danger')
            return render_template('test_form.html', available_grades=available_grades)
        ...
        db.session.commit()
        flash(f'Đã tạo bài kiểm tra "{title}" cho {grade}.', 'success')
        return redirect(url_for('manage_test_exam', exam_id=exam.id))

    return render_template('test_form.html', available_grades=available_grades)


@app.route('/tests/<int:exam_id>/edit', methods=['GET', 'POST'])
@login_required
def edit_test_exam(exam_id):
    if current_user.role not in ['admin', 'giaovien']:
        flash('Bạn không có quyền truy cập trang này.', 'danger')
        abort(403)
...
def delete_test_exam(exam_id):
    ...
    db.session.commit()
    flash(f'Đã xóa bài kiểm tra "{exam_title}".', 'success')
    return redirect(url_for('test_management'))
...
def add_test_attempt(exam_id):
    ...
    if not student_ids:
        flash('Bài kiểm tra này chưa có học sinh để thêm lần thi.', 'danger')
        return redirect(url_for('manage_test_exam', exam_id=exam.id))
    ...
    flash(f'Đã thêm {attempt_count} lần kiểm tra mới.', 'success')
...
def delete_test_attempt(exam_id, attempt):
    ...
    if attempt not in attempt_numbers:
        flash('Không tìm thấy lần kiểm tra cần xóa.', 'danger')
        return redirect(url_for('manage_test_exam', exam_id=exam.id))

    if len(attempt_numbers) <= 1:
        flash('Bài kiểm tra phải còn ít nhất 1 lần.', 'danger')
    ...
    flash(f'Đã xóa lần {attempt} và tự động dồn các lần phía sau.', 'success')
    return redirect(url_for('manage_test_exam', exam_id=exam.id, attempt=redirect_attempt))


@app.route('/tests/<int:exam_id>/export_csv')
@login_required
def export_test_exam_csv(exam_id):
    return redirect(url_for(
        'export_test_exam_csv_internal',
        exam_id=exam_id,
        attempt=request.args.get('attempt', type=int),
...
def export_test_exam_csv_internal(exam_id):
    ...
    rows = build_sorted_test_rows(results, sort_by=sort_by, sort_order=sort_order)

    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(['Tên học sinh', 'Điểm', 'Nhận xét'])
...
def manage_test_exam(exam_id):
    ...
    if request.method == 'POST':
        try:
            ...
            if has_changes:
                db.session.commit()
                flash(f'Đã lưu {format_test_attempt_label(attempt)} cho bài kiểm tra.', 'success')
            else:
                flash('Không có thay đổi nào để lưu.', 'info')
        except ValueError:
            db.session.rollback()
            flash('Điểm không hợp lệ. Chỉ nhập số từ 0 đến 10 và dùng dấu "." như 6.5 hoặc 6.75.', 'danger')
...
def export_public_test_scores_csv():
    ...
    if not selected_exam_id:
        flash('Vui lòng chọn bài kiểm tra để xuất file.', 'warning')
    ...
    score_entries = TestScore.query.filter_by(test_exam_id=selected_exam.id).all()
    student_ids = sorted({entry.student_id for entry in score_entries})
    students_map = {
        student.id: student
    ...
    for entry in score_entries:
        results_by_student.setdefault(entry.student_id, {})[entry.attempt_number] = entry
    ...
    if attempt_filter == 'all':
        writer.writerow([
            'Tên học sinh',
            *[build_test_attempt_labels(attempt_numbers)[attempt_number] for attempt_number in attempt_numbers]
        ])
    else:
        writer.writerow(['Tên học sinh', 'Điểm', 'Nhận xét'])
    ...
    for student_id in student_ids:
        ...
        attempts = results_by_student.get(student_id, {})
        if attempt_filter == 'all':
            writer.writerow([
                student.name,
                *[
                    '' if not attempts.get(attempt_number) or attempts[attempt_number].score is None else attempts[attempt_number].score
                    for attempt_number in attempt_numbers
                ]
            ])
        else:
            attempt_number = int(attempt_filter)
            entry = attempts.get(attempt_number)
            score_text = '' if not entry or entry.score is None else entry.score
            comment_text = entry.comment if entry and entry.comment else ''
            writer.writerow([student.name, score_text, comment_text])

    attempt_suffix = attempt_filter if attempt_filter != 'all' else 'all'
    filename = f'public_test_scores_exam_{selected_exam.id}_{attempt_suffix}.csv'
    return send_file(
        io.BytesIO(output.getvalue().encode('utf-8-sig')),
        mimetype='text/csv',
        as_attachment=True,
...
def public_test_scores():
    ...
    if selected_exam:
        ...
        for student_id in student_ids:
            ...
            attempts = results_by_student.get(student_id, {})
            latest_score = None
            for attempt_number in reversed(attempt_numbers):
        ...
        if sort_by == 'score':
            if attempt_filter != 'all' and attempt_filter.isdigit():
                attempt_number = int(attempt_filter)
                if sort_order == 'asc':
                    rows.sort(key=lambda row: (
                        row['attempts'].get(attempt_number) is None or row['attempts'][attempt_number].score is None,
                        row['attempts'][attempt_number].score if row['attempts'].get(attempt_number) and row['attempts'][attempt_number].score is not None else 0,
                        row['student'].name or ''
                    ))
                else:
                    rows.sort(key=lambda row: (
                        row['attempts'].get(attempt_number) is None or row['attempts'][attempt_number].score is None,
                        -(row['attempts'][attempt_number].score if row['attempts'].get(attempt_number) and row['attempts'][attempt_number].score is not None else -1),
                        row['student'].name or ''
                    ))
            else:
                if sort_order == 'asc':
                    rows.sort(key=lambda row: (
                        row['latest_score'] is None,
                        row['latest_score'] if row['latest_score'] is not None else 0,
                        row['student'].name or ''
                    ))
                else:
                    rows.sort(key=lambda row: (
